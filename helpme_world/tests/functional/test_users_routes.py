...
def test_account_route(test_client):
    ...
    response = test_client.post('/account', follow_redirects=True)
    assert response.status_code == 200
...
def test_user_route(test_client, new_user):
    ...
    THEN check that the response is is METHOD NOT ALLOWED (405)
    """
    response = test_client.post('/user/'+ str(new_user.username))
    assert response.status_code == 405
...
def test_password_token_route(test_client):
    ...
    assert response.status_code == 302
    response = test_client.post('/reset_password/<token>', follow_redirects=True)
    assert response.status_code == 200